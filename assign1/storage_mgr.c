...
RC readBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage)
{
	...
	if ( file == NULL)
    {
        ...
        if(pageNum < 0 || pageNum > fHandle->totalNumPages - 1)//if page number less than 0 or page number greater than total pages then error
        {
...
RC writeBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage){
    ...
    file = fopen(fHandle->fileName, "w+");
    if (file == NULL){
        //checks if file is found or not
        return RC_FILE_NOT_FOUND;
    }
    else if (file != NULL && pageNum > 0 && pageNum <= fHandle->totalNumPages){
